...
void Relation::loadRelation(const char* fileName)
{
  ...
  for (unsigned int i = 0; i < numColumns; ++i) {
      // for each column, find upper bound and lower bound of column.
      int hi = MIN_VAL;
      int lo = MAX_VAL;
      ...
      // create profile
      this->profiles.push_back(Profile(hi, lo, size, columns.size()));
      // fill bucket for each column.
      this->profiles[i].fill_bucket(columns[i], size);
