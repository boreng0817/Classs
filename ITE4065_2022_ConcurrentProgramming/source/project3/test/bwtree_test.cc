...
TEST_F(BwtreeTest, LongRandomInsertAndRead) {
  // This defines the key space (0 ~ (8M - 1))
  const uint32_t key_num = 8 * 1024 * 1024;
  std::atomic<size_t> insert_success_counter = 0;
  ...
  // Inserts in a 8M key space randomly until all keys has been inserted
  // Also, verify key is actually in the tree while doing Insert.
  auto workload = [&](uint32_t id) {
    const uint32_t gcid = id + 1;
    tree->AssignGCID(gcid);
    std::default_random_engine thread_generator(id);
    ...
    while (insert_success_counter.load() < key_num) {
      ...
      auto s = tree->GetValue(key);

      EXPECT_EQ(s.size(), 1);
      EXPECT_EQ(*s.begin(), key);
    }
    tree->UnregisterThread(gcid);
  };
...
TEST_F(BwtreeTest, SkewInsertAndReadTest) {
  // This defines the key space (0 ~ (64K - 1))
  const uint32_t key_num = 64 * 1024;
  std::atomic<size_t> insert_success_counter = 0;
  ...
  auto workload = [&](uint32_t id) {
    ...
    double alpha = 1.0;
    auto zipf = util::Zipf(alpha, key_num);
    zipf.SetSeed(id + 5);

    while (insert_success_counter.load() < key_num) {
      int key = zipf.Generate() - 1;
