...
class BwTreeBase {
 ...
 public:
   ...
   */
  NO_ASAN BwTreeBase() : gc_metadata_p{nullptr}, original_p{nullptr}, thread_num{total_thread_num.load()}, epoch{0UL}, minimum_epoch{0UL} {
  ...
  /*
   * GetGlobalMinEpoch() - Returns the current global minimum epoch counter
   *
   */
  NO_ASAN inline uint64_t GetGlobalMinEpoch() { return minimum_epoch; }
  ...
  // when main thread calls UpdateThreadLocal(). If downsizing occurs,
