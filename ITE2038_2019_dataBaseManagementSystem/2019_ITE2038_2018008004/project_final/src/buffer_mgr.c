...
buffer buf_init(int size) {
    ...
    for (i = 0; i < size; ++i) {
        ...
        buf -> frames[i].buf_page_mutex = PTHREAD_MUTEX_INITIALIZER;
        buf -> frames[i].acquired = false;
    }
    ...
    buf -> buf_pool_mutex = PTHREAD_MUTEX_INITIALIZER;
...
/* wrapper function of buf_read_page.
 * for join project, get frame pointer for join operation.
 */
frame *buf_get_frame(tablenum_t table_num, pagenum_t page_num) {

    return &(g_buf -> frames[ buf_read_page(table_num, page_num, NULL) ]);
}
...
void buf_evict_frame(int index) {
    ...
    if (index == -1) {
        ...
        while (frame_index_evict != LRU_LIST_TAIL &&
               ...
               !BUF_GET_FRAME(g_buf, frame_index_evict).acquired) { // neccessary?
...
/* pinning a frame for not to be evicted.
 */
...
/* free g_buf when program calls shutdown_db.
 */
void buf_free() {
