...
void print_list(lock_list_t *list) {
    ...
    while (iter != LOCK_LIST_TAIL) {
        printf("trx_id[%d]rec[%d]acq[%d] %s", iter -> trx -> trx_id, 
                    iter -> record_id ,iter -> acquired, iter->tail ? "|tail|   " : "" );
        iter = iter -> next;
    }
    printf("\n");
}
...
int db_update(int table_id, int64_t key, char* values, int trx_id) {
    ...
    while (1) {
        ...
        else if (rl_state == CONFLICT) { cout << "trx id[" << trx_id << "] waits\n";
            release_buf_page_latch(frame_index);
            trx_wait_trx(trx_id);
            continue;
        }
