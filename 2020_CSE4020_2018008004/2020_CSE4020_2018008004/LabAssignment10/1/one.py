...
def exp(rv):
    ...
    r1 = [cos+r[0]*r[0]*(1-cos), r[0]*r[1]*(1-cos)-r[2]*sin, 
            r[0]*r[2]*(1-cos)+r[1]*sin]
    r2 = [r[1]*r[0]*(1-cos)+r[2]*sin, cos+r[1]*r[1]*(1-cos),
            r[1]*r[2]*(1-cos)-r[0]*sin]
    r3 = [r[2]*r[0]*(1-cos)-r[1]*sin, r[2]*r[1]*(1-cos)+r[0]*sin,
            cos+r[2]*r[2]*(1-cos)]
...
"""
    ...
    for i in range(3):
        for j in range(3):
            if i == j:
                R[i][i] = cos + rv[i]*rv[i]*(1-cos)
            else:
                R[i][j] = rv[i]*rv[j]*(1-cos)
    R[0][1] -= rv[2]*sin
    R[0][2] += rv[1]*sin
    R[1][0] += rv[2]*sin
    R[1][2] -= rv[0]*sin
