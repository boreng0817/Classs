...
get_choice(uint bn)
{
	uint block_size[] = {NDIRECT, NINDIRECT, NDUINDIRECT, NTRIINDEIECT};
	int i;
...
bmap(struct inode *ip, uint bn)
{
	...
	switch(choice) {
						 ...
						 //             ^^^^^
		case  2: bp = bread(ip->dev, addr);
						 a = (uint*)bp->data;
						 idx = bn;// - NDIRECT - NINDIRECT - NDUINDIRECT;
						 ...
						 //                    ^^^^^
		case  1: bp = bread(ip->dev, addr);
						 a = (uint*)bp->data;
						 idx = bn % NDUINDIRECT;// - NDIRECT - NINDIRECT;
						 ...
						 //                           ^^^
		case  0: bp = bread(ip->dev, addr);
						 a = (uint*)bp->data;
						 idx = bn % NINDIRECT;// - NDIRECT;
						 if((addr = a[idx]) == 0){
...
pwritei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  if(off + n < off)
    return -1;
	// when this write makes overflow on MAXFILE size.
  if(off + n > MAXFILE*BSIZE)
    return -1;
