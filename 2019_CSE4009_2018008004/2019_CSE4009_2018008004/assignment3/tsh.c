...
void eval(char *cmdline) 
{
    ...
    /* if input is builtin command,
     * execute and wait for another input.
     */
    if (builtin_cmd(argv))
        return;

    /* if input is not found in builtin function,
     ...
     */
        ...
        if (pid == 0) {
            ...
            /* try to execve, 
             * if success, just run w/ arguments and environ
        ...
        /* if bg flag is 1
         * run in BG and do addjob. Don't call waitfg
         * unblock SIGCHLD in parent as well after do addjob.
         */
        if (bg_flag) {
        ...
        /* if bg flag is 0
         * run in FG. wait the child process is finished.
         */
        else {
